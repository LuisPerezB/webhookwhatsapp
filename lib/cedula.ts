// =========================
// API CÉDULA ECUADOR
// Valida cédula y obtiene nombres del Registro Civil
// =========================
...
export async function validarCedulaAPI(cedula: string): Promise<DatosCedula> {
  // Validación matemática primero (módulo 10)
  ...
  try {
    ...
    if (!data?.nombres || !data?.apellidos) {
      // API respondió pero sin datos — cédula no encontrada en RC
      return {
        valida: false,
        cedula,
    ...
    // Si la API falla pero el dígito verificador es válido,
    // aceptamos la cédula pero sin nombre del RC
    return {
      valida: true,
      cedula,
...
// Algoritmo módulo 10 — validación matemática local
